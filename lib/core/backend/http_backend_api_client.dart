...
final class HttpBackendApiClient implements BackendApiClient {
  ...
  Uri _resolveUri(
    ...
    return resolvedUri.replace(
      queryParameters: Map<String, String>.of(queryParameters),
    );
  }
  ...
  Future<Map<String, String>> _buildHeaders(
    ...
    _removeHeaderIgnoreCase(headers, ApiContract.contentTypeHeader);
    _removeHeaderIgnoreCase(headers, ApiContract.authorizationHeader);

    headers[ApiContract.acceptHeader] = ApiContract.jsonContentType;

    if (hasBody) {
      headers[ApiContract.contentTypeHeader] = ApiContract.jsonContentType;
    }
        ...
        if (decoded is Map) {
          ...
          try {
            ...
            if (!envelope.isSuccess &&
                envelopeError != null &&
                envelopeError.code != ApiErrorCode.malformedPayload) {
              return _withHttpStatus(envelopeError, statusCode);
            }
          } on ApiException {
          ...
          try {
            ...
            if (rawError.code != ApiErrorCode.unknown) {
              return _withHttpStatus(rawError, statusCode);
            }
          } on ApiException {
  ...
  static String _messageForStatus(int statusCode) {
    return switch (_errorCodeForStatus(statusCode)) {
      ...
      ApiErrorCode.conflict => 'The request conflicts with current state.',
      ApiErrorCode.validationFailed => 'The request could not be validated.',
      ApiErrorCode.rateLimited => 'Too many requests. Please try again later.',
      ApiErrorCode.serverError => 'The backend service returned an error.',
      _ => 'The backend returned an unexpected HTTP response.',
    };
  }
