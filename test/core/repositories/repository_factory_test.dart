...
isA<MockPurchaseRepository>(),
);
expect(repositories.adRepository, isA<MockAdRepository>());
expect(
repositories.leagueRepository,
isA<MockLeagueRepository>(),
);
expect(
repositories.knockoutRepository,
isA<MockKnockoutRepository>(),
);
});
...
expect(
(authRepository.apiClient as HttpBackendApiClient)
    .authSessionStore,
same(sessionStore),
);
...
test(
'backend runtime rejects transport with an injected API client',
() {
final factory = RepositoryFactory(
environment: const AppEnvironment.backend(
apiBaseUrl: 'https://api.test',
...
test(
'accepts an injected API client with its matching session store',
() {
final apiClient = _FakeBackendApiClient();
final sessionStore = InMemoryAuthSessionStore();
