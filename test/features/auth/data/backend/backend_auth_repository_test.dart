...
void main() {
  group('BackendAuthRepository registration and login', () {
    ...
    test('does not replace an existing session after failed or malformed '
        'registration', () async {
      ...
      final apiClient = _FakeBackendApiClient()
        ...
        ..enqueuePostSuccess({'playerProfile': _profilePayload()});

      final repository = _repository(apiClient, sessionStore);

      await expectLater(
        repository.register(username: 'Tester', password: 'pass123'),
        throwsA(
    ...
    test('rejects an expired session returned by registration', () async {
      final apiClient = _FakeBackendApiClient()
        ..enqueuePostSuccess(_authPayload(expiresAt: _pastExpiry));
      final sessionStore = InMemoryAuthSessionStore();
      final repository = _repository(apiClient, sessionStore);
    ...
    test(
      ...
      () async {
        ...
        await expectLater(
          repository.login(username: 'Tester', password: 'pass123'),
          throwsA(isA<AuthException>()),
        );
    ...
    test(
      ...
      () async {
        ...
        final apiClient = _FakeBackendApiClient()..enqueuePostSuccess(payload);
    ...
    test('logs in, maps the profile, and saves the returned session', () async {
      final apiClient = _FakeBackendApiClient()
        ..enqueuePostSuccess(_authPayload());
      final sessionStore = InMemoryAuthSessionStore();
      final repository = _repository(apiClient, sessionStore);
    ...
    test('maps session save failure to AuthException', () async {
      ...
      final sessionStore = _FailingAuthSessionStore(saveError: storageFailure);
    ...
    test('keeps an existing valid session after failed login', () async {
      final existingSession = _existingSession();
      final sessionStore = InMemoryAuthSessionStore();
      await sessionStore.save(existingSession);
    ...
    test(
      ...
      () async {
        ...
        for (final entry in expectedMessages.entries) {
          final apiClient = _FakeBackendApiClient()
            ..enqueuePostFailure(
              ApiError(code: entry.key, message: 'Internal backend detail.'),
            );

          final repository = _repository(apiClient, InMemoryAuthSessionStore());
    ...
    test('maps an ApiException thrown directly during login', () async {
      final existingSession = _existingSession();
      final sessionStore = InMemoryAuthSessionStore();
      await sessionStore.save(existingSession);
    ...
    test('login only rejects empty credentials locally', () {
      final apiClient = _FakeBackendApiClient();
      final repository = _repository(
        apiClient,
        InMemoryAuthSessionStore(),
  ...
  group('BackendAuthRepository current authentication state', () {
    test('maps session read failure to AuthException', () async {
      final storageFailure = AuthSessionStoreException(
        operation: AuthSessionStoreOperation.read,
        cause: Exception('secure storage unavailable'),
      );

      final sessionStore = _FailingAuthSessionStore(readError: storageFailure);

      final repository = _repository(_FakeBackendApiClient(), sessionStore);
      ...
      expect(sessionStore.readCalls, 1);
    });

    test('returns unauthenticated when no stored session exists', () async {
      final repository = _repository(
    ...
    test(
      'clears an expired session without refresh token and returns unauthenticated',
      () async {
        final store = InMemoryAuthSessionStore();
        await store.save(_expiredSession());

        final repository = _repository(_FakeBackendApiClient(), store);
    ...
    test(
      ...
      () async {
        for (final code in [
          ...
          final apiClient = _FakeBackendApiClient()
            ..enqueueGetFailure(ApiError(code: code, message: 'Denied.'));
    ...
    test('preserves session for network failure responses', () async {
    ...
    test('preserves session for malformed profile payloads', () async {
      final session = _existingSession();
      final store = InMemoryAuthSessionStore();
      await store.save(session);

      final apiClient = _FakeBackendApiClient()
        ..enqueueGetSuccess({'id': 'player-1'});
  ...
  group('BackendAuthRepository logout', () {
    test('maps session clear failure during logout to AuthException', () async {
      final existingSession = _existingSession();
      ...
      final sessionStore = _FailingAuthSessionStore(
        session: existingSession,
        clearError: storageFailure,
      );
      ...
      // A limpeza falhou, portanto o fake mantém a sessão anterior.
      expect(sessionStore.session, same(existingSession));
...
AuthSession _existingSession() {
  return AuthSession(accessToken: 'existing-token', expiresAt: _futureExpiry);
}

AuthSession _expiredSession() {
  return AuthSession(accessToken: 'expired-token', expiresAt: _pastExpiry);
}

Map<String, Object?> _authPayload({DateTime? expiresAt}) {
  return {
